...
/* USER CODE BEGIN APP_BLE_Init_1 */
	UTIL_SEQ_RegTask(1 << CFG_TASK_DO_SEND, UTIL_SEQ_RFU, doSend);
...
void BLE_SVC_L2CAP_MinInterval(void) {
	if (BleApplicationContext.Device_Connection_Status != APP_BLE_FAST_ADV
			&& BleApplicationContext.Device_Connection_Status != APP_BLE_IDLE) {
		uint16_t interval_min = 0x6;
		uint16_t interval_max = 0x6;
		uint16_t slave_latency = 0x0;
		uint16_t timeout_multiplier = 0x190;
		tBleStatus result;
		...
		result =
				aci_l2cap_connection_parameter_update_req(
						...
						timeout_multiplier);
		if (result == BLE_STATUS_SUCCESS) {
			APP_DBG_MSG("BLE_SVC_L2CAP_Min_Interval(), Successfully \r\n\r");
		} else {
			APP_DBG_MSG("BLE_SVC_L2CAP_Min_Interval(), Failed \r\n\r");
		}
	}
}
void BLE_SVC_GAP_UpdateConnTo2M(void) {
	tBleStatus result;
		result = hci_le_set_phy(BleApplicationContext.BleApplicationContext_legacy.connectionHandle,ALL_PHYS_PREFERENCE,TX_2M_PREFERRED,RX_2M_PREFERRED,0);
		if (result == BLE_STATUS_SUCCESS) {
			APP_DBG_MSG("BLE_SVC_GAP_UpdateConnTo2M(), Successfully \r\n\r");
		} else {
			APP_DBG_MSG("BLE_SVC_GAP_UpdateConnTo2M(), Failed \r\n\r");
		}
}
